...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->bist_en_gpio)) {
		rc = gpio_request(r_config->bist_en_gpio, "bist_en_gpio");
		if (rc) {
			DSI_ERR("request for bist_en_gpio failed, rc=%d\n", rc);
	...
	u32 bl_lvl, u8 hbm)
{
	...
	rc = mipi_dsi_dcs_set_display_brightness(dsi, bl_lvl, hbm);
	if (rc < 0)
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	panel->reset_config.bist_en_gpio = utils->get_named_gpio(utils->data,
						"qcom,bist-en-gpio",
						0);
	if (!gpio_is_valid(panel->reset_config.bist_en_gpio)) {
		rc = panel->reset_config.bist_en_gpio;
		DSI_ERR("[%s] failed get bist-en gpio, rc=%d\n", panel->name, rc);
	}
...
#if 0
	if (gpio_is_valid(panel->reset_config.bist_en_gpio)) {
		rc = gpio_direction_output(panel->reset_config.bist_en_gpio, 1);
		if (rc) {
			DSI_ERR("unable to set dir for bist-en gpio rc=%d\n", rc);
			//goto exit;
		}
	}
#endif
