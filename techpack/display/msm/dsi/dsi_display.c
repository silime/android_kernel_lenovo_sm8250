...
/*	
	if(display->esd_flag == -1){
		DSI_ERR("esd_flag irq detected, rc return -1 to recover\n");
		display->esd_flag = 1;
		//return -1;
	}else{
		display->esd_flag = 1;
		...
		void *data)
{
	...
	display->drm_dev = drm;
        ...
        if(register_hardware_info("OLED", "SAMSUNG, AMSA26ZP01")) {
...
/*
static void dsi_display_esd_err_request_gpio(struct dsi_display *display) 
{
    struct platform_device *pdev;
	struct device *dev;
	int rc = 0;
    ...
    display->mipi_err_gpio = of_get_named_gpio(dev->of_node, 
					"qcom,platform-mipi-err-gpio", 0);
    if (!gpio_is_valid(display->mipi_err_gpio)) {
        DSI_ERR("mipi_err_gpio is not specified.\n");
        return;
    }
    rc = gpio_request_one(display->mipi_err_gpio, GPIOF_DIR_IN | GPIOF_EXPORT, "mipi_err_gpio");
    if (rc) {
        DSI_ERR("Fail to request mipi_err_gpio [%d]\n", display->mipi_err_gpio);
        display->mipi_err_gpio = -1;
    }
...
static void esd_err_flag_check_work(struct work_struct *work)
{
	struct dsi_display *display = container_of(work, struct dsi_display, esd_err_flag_check_work);
	DSI_ERR("%s : esd is detected, set flag to -1....", __func__);	
...
static irqreturn_t esd_err_handler(int irq, void *dev_id)
{
	struct dsi_display *display = dev_id;
	pm_stay_awake(&display->pdev->dev);
	schedule_work(&display->esd_err_flag_check_work);
	return IRQ_HANDLED;
}
*/
...
/*
	device_init_wakeup(&display->pdev->dev, 1);
	display->esd_flag = 1; //1: normal, -1:esd happened
	dsi_display_esd_err_request_gpio(display);	
	INIT_WORK(&display->esd_err_flag_check_work, esd_err_flag_check_work);
	display->esd_err_flag_irq = gpio_to_irq(display->mipi_err_gpio);
	ret = devm_request_threaded_irq(&pdev->dev, display->esd_err_flag_irq, NULL, esd_err_handler, IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "mipi_err_gpio", display);
	if(ret < 0){
		DSI_ERR("esd_err request irq failed\n");
	}
*/	
